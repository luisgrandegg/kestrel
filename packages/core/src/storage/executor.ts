 ...
 *
 * ADAPTER REQUIREMENT: UPDATE statements must report a real count — the
 * repository's unknown-ticker detection depends on it and fails loud on
 * `null` (a driver/adapter that omits counts, e.g. PostgREST without an
 * explicit count preference, is not a conforming backend for this seam).
 */
